...
let rec ident opt_allowed tgt i =
  ...
  | Ast0.MetaIdWithParent((name,constraints,seed,pure), (name1,constraints1,seed1,pure1)) ->
      let arity =
	all_same opt_allowed tgt (mcode2line name)
	  [mcode2arity name] in
      let name = mcode name in
      make_id i tgt arity ( Ast0.MetaIdWithParent((name,constraints,seed,pure), (name1,constraints1,seed1,pure1)))
